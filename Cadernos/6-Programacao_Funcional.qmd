...
-   **Gestão hospitalar**: extrair e resumir informações de pacientes internados em diferentes unidades (clínica médica, UTI, pediatria) sem utilizar loops

    ```{r}

    pacientes <- data.frame(
      id = 1:10,
      nome = paste("Paciente", 1:10),
      idade = c(34, 78, 45, 2, 65, 28, 50, 1, 72, 38),
      sexo = c("F", "M", "M", "F", "F", "M", "F", "M", "F", "M"),
      unidade = c("Clínica Médica", "UTI", "Clínica Médica", "Pediatria", "Clínica Médica", "UTI", "UTI", "Pediatria", "Clínica Médica", "Pediatria"),
      tempo_internacao_dias = c(5, 12, 4, 3, 7, 10, 15, 2, 9, 6),
      desfecho = c("Alta", "Óbito", "Alta", "Alta", "Alta", "Óbito", "Alta", "Alta", "Alta", "Alta")
    ) 
    pacientes
    ```

-   **Análise de variáveis climáticas**: resumir informções sobre temperaturas, umidade relativa e quantidade de chuvas sem utilizar loops

    ```{r}
    dados = data.frame(
      Temperatura = c(30,32,25,28),
      Umidade = c(0.61,0.63,0.80,0.70)
    ) 
    dados
    ```

-   Resolução de um problema maior apartir m
